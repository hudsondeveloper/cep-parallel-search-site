...
export default function CepValidator() {
  ...
  return (
    <div className="relative py-16 sm:py-20 md:py-24 px-4 sm:px-6 bg-gradient-to-b from-transparent via-green-950/20 to-transparent">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="relative group mb-6 sm:mb-8">
          ...
          <div className="relative bg-slate-900/90 backdrop-blur-xl rounded-2xl border border-white/10 p-4 sm:p-6 md:p-8">
            <div className="space-y-3 sm:space-y-4">
              <label htmlFor="cep-input" className="block text-xs sm:text-sm font-medium text-slate-300 mb-2">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <input
                  ...
                  className="flex-1 px-3 sm:px-4 py-2.5 sm:py-3 bg-slate-800/50 border border-white/10 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-green-500/50 focus:border-green-500/50 transition-all text-sm sm:text-base"
                />
                <button
                  onClick={handleValidate}
                  className="w-full sm:w-auto px-6 py-2.5 sm:py-3 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-500 hover:to-emerald-500 text-white font-semibold rounded-xl shadow-lg shadow-green-500/25 hover:shadow-green-500/40 transition-all duration-300 text-sm sm:text-base"
                >
                  Validar
                </button>
              </div>
              ...
              {/* Result */}
              {isValid !== null && (
                <div className={`mt-3 sm:mt-4 p-3 sm:p-4 rounded-xl border ${
                  isValid 
                  ...
                  <div className="flex items-start gap-2 sm:gap-3">
                    {isValid ? (
                      <>
                        <Check className="w-5 h-5 sm:w-6 sm:h-6 text-green-400 flex-shrink-0 mt-0.5" />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm sm:text-base text-green-400 font-semibold">CEP Válido!</p>
                      ...
                      <>
                        <X className="w-5 h-5 sm:w-6 sm:h-6 text-red-400 flex-shrink-0 mt-0.5" />
                        <div className="flex-1 min-w-0">
                          <p className="text-sm sm:text-base text-red-400 font-semibold">CEP Inválido</p>
                          <p className="text-xs sm:text-sm text-slate-400 mt-1">
              ...
              {cep.trim() === '' && (
                <div className="mt-3 sm:mt-4 p-3 sm:p-4 rounded-xl bg-blue-500/10 border border-blue-500/30">
                  <div className="flex items-start gap-2 sm:gap-3">
                    ...
                    <div className="text-xs sm:text-sm text-slate-300 flex-1 min-w-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          {examples.map((example, index) => (
            <div
              key={index}
              className="bg-slate-900/90 backdrop-blur-xl rounded-xl border border-white/10 p-3 sm:p-4 hover:border-white/20 transition-all cursor-pointer"
              onClick={() => {
                setCep(example.cep);
            ...
            >
              <div className="flex items-center justify-between mb-1.5 sm:mb-2 gap-2">
                <code className="text-blue-400 font-mono text-xs sm:text-sm break-all">{example.cep}</code>
                {example.valid ? (
              ...
              <p className="text-xs sm:text-sm text-slate-400">{example.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8 sm:mt-12 relative group">
          ...
          <div className="relative bg-slate-900/90 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden">
            <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-white/10">
              <span className="text-xs sm:text-sm font-medium text-slate-300">Exemplo de Código</span>
            </div>
